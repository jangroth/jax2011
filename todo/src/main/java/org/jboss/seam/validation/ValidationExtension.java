...
public class ValidationExtension implements Extension {
	...
	private void addValidatorFactoryIfRequired(AfterBeanDiscovery abd,
			final BeanManager beanManager) {

		// if a ValidatorFactory already exists, only inject it's
		// ConstraintValidatorFactory if required
		if (!beanManager.getBeans(ValidatorFactory.class).isEmpty()) {

			ValidatorFactory validatorFactory = getReference(beanManager,
					ValidatorFactory.class);

			ConstraintValidatorFactory constraintValidatorFactory = validatorFactory
					.getConstraintValidatorFactory();
			if (constraintValidatorFactory instanceof InjectingConstraintValidatorFactory) {
				inject(beanManager,
						InjectingConstraintValidatorFactory.class,
		...
		abd.addBean(new Bean<ValidatorFactory>() {
			...
			@Override
			public void destroy(ValidatorFactory instance,
					CreationalContext<ValidatorFactory> ctx) {
	...
	private void addValidatorIfRequired(AfterBeanDiscovery abd,
		...
		abd.addBean(new Bean<Validator>() {
			...
			public Validator create(CreationalContext<Validator> ctx) {
				...
				ValidatorFactory validatorFactory = getReference(bm,
						ValidatorFactory.class);
			...
			@Override
			public void destroy(Validator instance,
	...
	private void addMethodValidator(AfterBeanDiscovery abd, final BeanManager bm) {
		...
		abd.addBean(new Bean<MethodValidator>() {
			...
			@Override
			public void destroy(MethodValidator instance,
					CreationalContext<MethodValidator> ctx) {
	...
	private <T> void inject(final BeanManager beanManager, Class<T> type,
			T constraintValidatorFactory) {

		AnnotatedType<T> annotatedType = beanManager.createAnnotatedType(type);
		InjectionTarget<T> it = beanManager
